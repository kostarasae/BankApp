...
-- ============================================================
-- 1. CAN_TRANSFER capability (missing from V3)
-- ============================================================
INSERT IGNORE INTO capabilities (name, description)
VALUES ('CAN_TRANSFER', 'Transfer funds between accounts');

INSERT IGNORE INTO roles_capabilities (role_id, capability_id)
SELECT r.id, c.id FROM roles r JOIN capabilities c
WHERE r.name IN ('ADMIN', 'CUSTOMER') AND c.name = 'CAN_TRANSFER';
...
-- ============================================================
-- 3. Users  (role ids: ADMIN=1, EMPLOYEE=2, CUSTOMER=3 per V3)
-- ============================================================
...
VALUES
    ('GR1600000000000000000000001', 500.00, 'DEPOSIT',    'Κατάθεση ATM Σύνταγμα',      DATE_SUB(NOW(), INTERVAL 5 DAY), DATE_SUB(NOW(), INTERVAL 5 DAY)),
    ('GR1600000000000000000000001', 850.00, 'DEPOSIT',    'Μισθός Απριλίου',             DATE_SUB(NOW(), INTERVAL 1 DAY), DATE_SUB(NOW(), INTERVAL 1 DAY)),
    ('GR1600000000000000000000001', 150.00, 'WITHDRAWAL', 'Ανάληψη ATM Ομόνοια',        DATE_SUB(NOW(), INTERVAL 4 DAY), DATE_SUB(NOW(), INTERVAL 4 DAY)),
    ('GR1600000000000000000000001', 300.00, 'WITHDRAWAL', 'Ανάληψη κατάστημα',          DATE_SUB(NOW(), INTERVAL 2 DAY), DATE_SUB(NOW(), INTERVAL 2 DAY)),
    ('GR1600000000000000000000001', 100.00, 'TRANSFER',   'IRIS μεταφορά προς Νίκο',    DATE_SUB(NOW(), INTERVAL 3 DAY), DATE_SUB(NOW(), INTERVAL 3 DAY)),
    ('GR1600000000000000000000001', 200.00, 'TRANSFER',   'ΔΕΗ πληρωμή',                DATE_SUB(NOW(), INTERVAL 1 DAY), DATE_SUB(NOW(), INTERVAL 1 DAY));

-- Μαρία — ταμιευτήριο (GR...002)
INSERT INTO transactions (iban, amount, type, description, created_at, updated_at)
VALUES
    ('GR1600000000000000000000002', 500.00, 'DEPOSIT', 'Αρχικό ποσό',  DATE_SUB(NOW(), INTERVAL 10 DAY), DATE_SUB(NOW(), INTERVAL 10 DAY)),
    ('GR1600000000000000000000002', 200.00, 'DEPOSIT', 'Εξοικονόμηση', DATE_SUB(NOW(), INTERVAL  3 DAY), DATE_SUB(NOW(), INTERVAL  3 DAY));

-- Νίκος — τρεχούμενος (GR...003)
INSERT INTO transactions (iban, amount, type, description, created_at, updated_at)
VALUES
    ('GR1600000000000000000000003', 300.00, 'DEPOSIT',    'Κατάθεση ATM Θεσσαλονίκη',  DATE_SUB(NOW(), INTERVAL 6 DAY), DATE_SUB(NOW(), INTERVAL 6 DAY)),
    ('GR1600000000000000000000003', 100.00, 'DEPOSIT',    'IRIS μεταφορά από Μαρία',    DATE_SUB(NOW(), INTERVAL 3 DAY), DATE_SUB(NOW(), INTERVAL 3 DAY)),
    ('GR1600000000000000000000003', 150.00, 'WITHDRAWAL', 'Ανάληψη ATM Μοναστηράκι',   DATE_SUB(NOW(), INTERVAL 1 DAY), DATE_SUB(NOW(), INTERVAL 1 DAY));
